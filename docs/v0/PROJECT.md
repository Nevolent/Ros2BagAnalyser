# ROS 2 Bag Analyser — V0 Project Definition

**Status:** Building blocks 1–5 complete and user-accepted; mentor-facing V0
complete; post-V0 Building block 8 implemented and verified, awaiting review

**Repository baseline:** Completed mentor-facing V0
...
8. View a selected raw IMU motion component following the same timeline.
...
## 9. Post-V0 common-sensor extension

The read-only 2026-07-30 archive audit found one standard numeric sensor stream
with an exact common name, type, CDR serialization, and positive message count
across every metadata file and all five readable databases:

```text
/kuupkulgur_v1/sensors/imu0/raw_data
sensor_msgs/msg/Imu
```

The five readable databases contain 184,810 messages on that topic. Complete
immutable inspection found finite values and no duplicate ROS record timestamps
for these six raw components:

- `angular_velocity.x`, `.y`, and `.z` in radians per second;
- `linear_acceleration.x`, `.y`, and `.z` in metres per second squared.

Orientation is not an available measurement in this archive:
`orientation_covariance[0]` is `-1` throughout the five readable streams. The
constant identity quaternion must therefore not be graphed or converted into
Euler angles.

The exact common topic set also contains `cmd_vel`, encoders, EPS state, the
front camera, and operational topics. `cmd_vel` is a command rather than a
sensor; encoder and EPS types require unavailable custom packages; the camera
already has its own pane; and the remaining topics are empty or operational.
They are not part of this extension. Ouster streams are not common to all
recordings and remain outside scope.

Building block 8 adds one selectable graph for the six approved IMU components.
The graph remains a view of the full recording clock and becomes an additional
seek surface for that clock. It does not introduce arbitrary topic selection,
multiple graphs, a graph-local time window, derived orientation, custom-message
processing, LiDAR, or commanded-motion telemetry.
