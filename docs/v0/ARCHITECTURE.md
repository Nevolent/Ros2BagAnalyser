# ROS 2 Bag Analyser — V0 Architecture

> **Status:** Accepted V0 architecture
>
> **Implementation status:** Building blocks 1–5 complete and user-accepted;
> V0 complete; post-V0 Building block 8 implemented and verified, awaiting
> user review
>
...
5. shows one selected IMU signal on the same timeline.
...
| Telemetry | Six fixed raw axes from one configured standard IMU topic |
...
## 15. Post-V0 selectable IMU extension

Building block 8 evolves the existing `imu_series` artifact instead of adding a
generic telemetry subsystem. The configured standard IMU topic is still the
only source, and the existing `imu_series` job kind, four PostgreSQL tables,
one serial worker, request/state routes, and identity-bound data delivery remain
unchanged.

### 15.1 Fixed series registry

The artifact contains exactly the six audited raw components:

- `angular_velocity.x`, `.y`, and `.z` in `rad/s`;
- `linear_acceleration.x`, `.y`, and `.z` in `m/s²`.

Names and units remain literal. The application does not infer rover-relative
axis names, Euler angles, or derived magnitudes. IMU orientation is excluded
when its covariance sentinel reports that the estimate is unavailable.

### 15.2 Bundle format and identity

Series schema version 2 stores one ordered row per source message. Each row
contains the decimal bag-relative record timestamp once, followed by the six
fixed finite-or-null values in registry order. The worker deserializes each IMU
message once, computes per-component counts and extrema, and publishes one
validated bounded JSON artifact.

Cache identity includes the fixed ordered registry, schema and processor
versions, source identities, exact topic contract, bag timing, configured
default component, null policy, duplicate policy, and reduction decision.
Version-1 artifacts remain app-owned history but do not satisfy a version-2
request.

### 15.3 Browser clock and graph seeking

The global full-recording clock remains the sole owner of time. The existing
global slider and the graph are two input surfaces for that same clock.
Pointer, touch, or keyboard graph seeking clamps to the recording duration,
force-seeks available camera consumers, updates the global slider and selected
IMU value, and reanchors monotonic playback. It does not create a graph-local
clock, window, pan, or zoom contract.

The static Canvas trace redraws only when data, selection, or plot dimensions
change. The moving cursor remains a separate overlay, uses device-pixel-snapped
composited translation, and coalesces pointer updates to one animation frame.
The existing 100-millisecond camera drift correction and measured-coverage
rules do not change.
