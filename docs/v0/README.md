...
and present two cameras and selectable raw IMU telemetry on a shared timeline.
...
model and one serial worker remain unchanged. Building block 6's frontend-only
reference UI migration and verification were completed on 2026-07-29 and are
awaiting user review. Building block 7's one-command local launcher and
real-archive development integration were approved, implemented, and verified
on 2026-07-30 and are awaiting user review.
Building block 8's six-channel IMU bundle, selectable graph, in-graph seek
surface, and flicker-resistant cursor were implemented and verified on
2026-07-30 and are awaiting user review.

The browser now uses the final served visual shell from the separately
inspected learning project: dark application chrome, database table, analyzer
grid, media and telemetry panes, one global timeline, and a metadata sidebar.
Only that visual language was carried over. The learning project is not a
runtime dependency or backend compatibility target; this application keeps its
saved-catalog startup, explicit rescan, asynchronous worker states, current API,
identity-bound artifact delivery, and accepted synchronization rules.
...
5. Show a synchronized IMU angular-velocity graph.
...
This development machine has a persistent local setup:

- the source archive is `/mnt/d/Rosbags`;
- generated output is under
  `/home/kardo/.local/share/rosbag-analyser/derived`;
- the automatic Windows PostgreSQL 16 service owns the dedicated
  `rosbag_analyser` database;
- private configuration is stored in
  `~/.config/rosbag-analyser/environment` and `database.env`, both with mode
  `0600`;
- `./dev` supervises one API and one serial worker with private PID and log
  state under `~/.local/state/rosbag-analyser`.

From the repository, start the complete application with:

```bash
./dev open
```

That command validates the roots and private configuration, applies migrations,
starts or reuses the API and worker, waits for both processes and HTTP health,
and opens `http://127.0.0.1:8000`. It does not scan the archive. Ordinary starts
load the saved PostgreSQL catalog.

The Windows desktop shortcut **ROS 2 Bag Analyser** runs the same command.
One-time installation or repair of the shortcut is:

```bash
./dev install
```

The remaining lifecycle commands are:

```bash
./dev start       # start without opening a browser
./dev status      # show API, worker, and HTTP health
./dev logs        # follow private API/worker logs; Ctrl+C leaves them running
./dev restart     # migrate and restart both processes
./dev stop        # stop the complete application
./dev migrate     # apply migrations without starting the app
./dev rescan      # explicitly perform the bounded read-only archive scan
./dev check       # validate configuration and root separation only
```

`./dev start` is safe to repeat. If the API is healthy but the worker is
stopped, it restores the worker instead of reporting a partial start as ready.
`./dev rescan` is intentionally explicit: launching the UI never scans, writes
beside, repairs, or reindexes an original rosbag.

Machine-specific configuration and secrets remain outside Git. Do not copy the
database password into a repository `.env` file. If startup fails, run
`./dev status`, then `./dev logs`; the Windows `postgresql-x64-16` service must
be running and `/mnt/d/Rosbags` must be mounted.
...
The front topic, default IMU component, and profile shown have defaults; the
IMU topic is required because topic names are recording/environment
configuration rather than a product constant. The default component may be
any supported raw angular-velocity or linear-acceleration axis and selects the
initial graph choice; all six axes are still extracted together.
`ROS_BAG_ANALYSER_FFMPEG` and
...
extracts the three raw `angular_velocity` axes and three raw
`linear_acceleration` axes in one source pass. ROS database record timestamps
are converted to integer nanoseconds relative to the bag start. The
schema-version-2 JSON bundle stores each timestamp once, preserves source
order and duplicates, and represents non-finite values as per-component
`null` gaps. The browser switches among the six exact signals without another
request or job, maps the selected signal across the full recording timeline,
and clears the current value outside measured coverage. The graph and global
range input seek the same clock.

The bundle remains unreduced: the 76,000-row synthetic six-channel profile
measured a 10.14 MB payload, about 110 ms to validate and parse, 34 ms for the
selected trace transform, and 6 ms for 10,000 timeline lookups on the
development host.
